	...
	function draw() {
		...
		for (var i = 0; i < particles.length; i++) {
			...
			if (PULSATION) { /* this would be 0 -> 1 */
				let mod = ((GLOBAL_PULSATION ? 0 : p.deltaOffset) + now) % PULSATION_PERIOD / PULSATION_PERIOD;

				/* lets make the value bouncing with sinus */
				mod = Math.sin(mod * Math.PI);
			} else var mod = 1;

			let offset = TREMBLING ? TREMBLING * (-1 + Math.random() * 2) : 0;

			let radius = PARTICLE_RADIUS * p.radius;
	...
	function get_destinations() {
		for (var i = 0; i < particles.length; i++) {
			...
			if (positions.length) {
				...
				let po1 = positions[nearest_position];
				for (var n = 0; n < positions.length; n++) {
					let po2 = positions[n];
